...
-(NSDictionary *)getDAEHttpCommonBodyParameters{
    //Add parameter to body if needed
    return nil;
}
...
-(ALActionResultWithData*)doRequestUsing:(ALHttpMethod)method with:(NSDictionary*)params andHeader:(NSDictionary *)header{
    ...
    //添加通用参数
    NSMutableDictionary *paramsDic = [[self getDAEHttpCommonBodyParameters] mutableCopy];
    if(params){
    ...
    funcResult = [alHttpRequest doRequestUsing:method with:paramsDic with:headerDic];
